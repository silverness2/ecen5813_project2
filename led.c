...
void set_led_blue_on()
{
    ...
    // PCOR (Port Clear Output) register. Corresponding bit in PDORn is set/cleared to logic 0.
    // Same as: GPIOD->PDOR &= ~0x02;
	GPIOD->PCOR = 0x02;
}
...
void set_led_blue_off()
{
	...
	// PSOR (Port Set Output) register. Corresponding bit in PDORn is set to logic 1.
    // Same as: GPIOD->PDOR |= 0x02;
	GPIOD->PSOR = 0x02;
}
