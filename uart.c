...
int uart_init()
{
    ...
    SIM->SCGC4 |= SIM_SCGC4_UART0_MASK; // mask = 0x400 = bit 10
    ...
    SIM->SCGC5 |= SIM_SCGC5_PORTA_MASK; // mask = 0x200 = bit 9
...
int uart_can_transmit()
{
    ...
    if ((UART0->S1 & UART0_S1_TDRE_MASK == 0) || // 0x80
        (UART0->S1 & UART0_S1_TC_MASK   == 0)) // 0x40
    {
